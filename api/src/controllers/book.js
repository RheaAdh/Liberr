...
exports.searchBooks = async (req, res) => {
    try {
        ...
        const availablebooks = await axios.get(
            'http://localhost:5000/api/books/availableBooks'
        );
            ...
            for (key in filters) {
                if (key == 'authors') {
                    for (author in book[key]) {
                        console.log(
                            filters[key],
                            filters[key] == book.authors[author],
                            book.authors[author]
                        );
                        if (filters[key] == book.authors[author]) {
                            isValid =
                                isValid && book.authors[author] == filters[key];
                            break;
                        }
                    }
                } else {
                    ...
                    isValid =
                        isValid &&
                        book[key].toUpperCase() == filters[key].toUpperCase();
                }
            }
            return isValid;
        });
